...
Try {
	...
	[string]$appVendor = 'SMSMarshall'
	[string]$appName = 'LogLauncher'
	[string]$appVersion = '3.6.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.2'
	[string]$appScriptDate = '2022.07.14'
	...
	$64bitAppInstallName = "LogLauncher"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\LogLauncher"
	# Application  install parameters.
	...
	$appSettingsDir = "$Env:ProgramData\Tuolumne County\Application Settings\LogLauncher"
	...
	# String to get the subnet, and then create string with only the necessary characters.
	[string]$IPAddress = Get-NetIPAddress
	$Subnet = $IPAddress.SubString(4, 7)

	# Installed application location
	$64bitAppInstallLocation = "$Env:Programfiles\LogLauncher"

	# Tracer path
	$TracerPath = "$PSScriptRoot\Files\LogLauncher\cmtrace.exe"

	# Tracer configurator path
	$TracerConfigPath = "$PSScriptRoot\SupportFiles\Tracer Path.exe"

	# Start menu shortcut path
	$MenuShortcutPath = "$Env:ProgramData\Microsoft\Windows\Start Menu\Programs\LogLauncher.lnk"

	# Desktop shortcut path
	$DesktopShortcutPath = "${Env:PUBLIC}\Desktop\LogLauncher.lnk"

	# Activesetup key name
	$SubKeyName = "LogLauncher - Setting Tracer Path"

	# Activesetup key name
	$KeyName = "HKCU:Software\SMSMarshall"

	# Activesetup key location
	$ActivesetupKey = "HKLM:SOFTWARE\Microsoft\Active Setup\Installed Components\$SubKeyName"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationProgress -StatusMessage "Removing previous versions. Please Wait..."

		# Check if the installation location exists, thus the application is installed.
		# If application is installed, remove the installation folder, start menu shortcut, desktop shortcut, activesetup key, and program data information.
		# If uninstall failed, log results. Exit script. 
		# Else, log results from check.
		If (Test-Path -Path "$64bitAppInstallLocation") {
			# Uninstall previous EXE versions of application(s).      
			Try {
				Remove-Folder -Path "$64bitAppInstallLocation"
				Remove-File -Path "$MenuShortcutPath"
				Remove-File -Path "$DesktopShortcutPath"
		...
		Try {
			...
			[scriptblock]$RegistryChanges = {
				# Remove the registry data location for the application.
				Remove-RegistryKey -Key "$KeyName" -Recurse -SID $UserProfile.SID
			}
		...
		#Paste executable and tracer in program files with the icon, then paste shortcut in start menu.
		# If installation failed, log results. Exit script. 
		Try {
			Write-log -Message "Installing $apName ($apversion)."
			Copy-Item -Path "$64bitAppInstallPath" -Destination "$64bitAppInstallLocation" -Force -Recurse
			New-Shortcut -Path "$MenuShortcutPath" -TargetPath "$64bitAppInstallLocation\$64bitAppInstallName.exe" -Description "$64bitAppInstallName" -IconLocation "$64bitAppInstallLocation\Assets\log_icon.ico"
		...
		Try {
			...
			Copy-File -Path "$TracerConfigPath" -Destination "$appSettingsDir" -Recurse
		}
		# If copy failed, log results. Exit script.
		Catch [System.Exception] {
		...
		Try {
			...
			Set-ActiveSetup -StubExePath "$appSettingsDir\Tracer Path.exe" -Description "$SubKeyName" -Key "$SubKeyName"
		...
		# Logs the subnet.
		# Creates desktop shortcut is the subnet matches the training room's.
		Write-Log -Message "Detected subnet: $Subnet"
		If ($Subnet -like "CONFIDENTIAL") {
			Write-Log -Message "Creating desktop shortcut."
			New-Shortcut -Path "$DesktopShortcutPath" -TargetPath "$64bitAppInstallLocation\$64bitAppInstallName.exe" -IconLocation "$64bitAppInstallLocation\Assets\log_icon.ico" -WindowStyle "Maximized" -Description "$64bitAppInstallName"
		}
